			...
			else
			{
				$points = $current_RET[$THIS_RET['STUDENT_ID']][$assignment_id][1]['POINTS'];                                
					...
					if($points!='-1')
                                        {
                                            if($points!='')
                                                 return ($THIS_RET['DUE']||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').(($points_m/$tot_point)*100).'%'.($THIS_RET['DUE']||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'</FONT>':''):'').'&nbsp;<B>'. _makeLetterGrade(($points_m/$tot_point)*100).'</B>'.($THIS_RET['DUE']||$points!=''?'':'</FONT>');
