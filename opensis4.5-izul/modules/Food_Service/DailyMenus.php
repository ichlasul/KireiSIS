...
else
{
	...
	if(count($calendars_RET))
		...
		ErrorMessage(array(get_string('thereAreNoCalendars...', 'Food_Service/DailyMenus')),'fatal');
}
...
if(!$_REQUEST['menu_id'])
	if(!$_SESSION['FSA_menu_id'])
		if(count($menus_RET))
			...
			ErrorMessage(array(get_string('thereAreNoMenus...', 'Food_Service/DailyMenus')),'fatal');
	else
...
else
{
	if(AllowEdit())
	{
		...
		{
			$description_select = '<OPTION value="">'.get_string('orSelectPreviousMeal', 'Food_Service/DailyMenus');
	...
	foreach($calendar_RET as $school_date=>$value)
		$events_RET[] = array('ID'=>get_string('new', 'Food_Service/DailyMenus'),'SCHOOL_DATE'=>ProperDate($school_date),'DESCRIPTION'=>TextInput('','food_service['.$school_date.'][text]','','size=35').($description_select ? '<SELECT name=food_service['.$school_date.'][select]>'.$description_select : ''));
	unset($events_RET[0]);
	$LO_columns = array('ID'=>'#','SCHOOL_DATE'=>get_string('date', 'Food_Service/DailyMenus'),'DESCRIPTION'=>get_string('description', 'Food_Service/DailyMenus'));
	...
	DrawHeader(PrepareDate(strtoupper(date("d-M-y",$time)),'',false,array('M'=>1,'Y'=>1,'submit'=>true)),SubmitButton(get_string('save', 'Food_Service/DailyMenus'),'submit[save]').'<INPUT type=submit value=\''.get_string('generateMenu', 'Food_Service/DailyMenus').'\' name=submit[print]>');
	echo '<BR>';
	...
	echo '<CENTER>'.SubmitButton(get_string('save', 'Food_Service/DailyMenus'),'submit[save]').'</CENTER>';
	echo "</FORM>";
}
