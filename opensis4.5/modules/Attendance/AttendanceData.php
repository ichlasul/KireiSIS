...
if(!$_REQUEST['modfunc'])
{
    ...
    if($_REQUEST['list_by_day']=='true')
    {
        ...
        $student_days_possible = DBGet(DBQuery("SELECT ap.SCHOOL_DATE, CONCAT(s.FIRST_NAME, ' ', s.LAST_NAME,' ') as STUDENTS, sg.TITLE as GRADE, sp.TITLE as PERIOD, ac.TITLE as STATUS from ATTENDANCE_PERIOD ap, STUDENTS s, SCHOOL_GRADELEVELS sg, ATTENDANCE_CODES ac, STUDENT_ENROLLMENT se, SCHOOL_PERIODS sp where ap.attendance_code=ac.id and se.grade_id=sg.id and ap.period_id=sp.period_id and ap.student_id=s.student_id and se.student_id=ap.student_id AND ap.SCHOOL_DATE BETWEEN '$start_date' AND '$end_date' ".$extra.CustomFields('where')." "),array('SCHOOL_DATE'=>'ProperDate','GRADE'=>'GRADE','STUDENTS'=>'STUDENTS','PRESENT'=>'_makeByDay','ABSENT'=>'_makeByDay','ADA'=>'_makeByDay'));
