...
if($_REQUEST['modfunc']=='remove')
{
	if($_REQUEST['tab_id']!='new')
	{
		if(DeletePrompt(get_string('category', 'Food_Service/Menus')))
		{
		...
		if(DeletePrompt(get_string('meal', 'Food_Service/Menus')))
		{
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['tab_id']!='new')
	{
		...
		$LO_columns = array('TITLE'=>get_string('mealCategory', 'Food_Service/Menus', $menus_RET[$_REQUEST['tab_id']][1]['TITLE']),'SORT_ORDER'=>get_string('sortOrder', 'Food_Service/Menus'));
	...
	else
	{
		...
		$functions = array('TITLE'=>'makeTextInput','SORT_ORDER'=>'makeTextInput');
		$LO_columns = array('TITLE'=>get_string('meal', 'Food_Service/Menus'),'SORT_ORDER'=>get_string('sortOrder', 'Food_Service/Menus'));
		...
		$singular = get_string('meal', 'Food_Service/Menus');
		$plural = get_string('meals', 'Food_Service/Menus');
	}
	$LO_ret = DBGet(DBQuery($sql),$functions);

	echo "<FORM action=Modules.php?modname=$_REQUEST[modname]&modfunc=update&tab_id=$_REQUEST[tab_id] method=POST>";
	DrawHeader('',SubmitButton(get_string('save', 'Food_Service/Menus')));
	echo '<BR>';
	...
	echo '<CENTER>'.SubmitButton(get_string('save', 'Food_Service/Menus')).'</CENTER>';
	echo '</FORM>';
}
