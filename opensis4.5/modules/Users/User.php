...
###########################################################################################################
                  /*     if(count($_REQUEST['food_service']))
			{
				...
				$sql = "INSERT INTO FOOD_SERVICE_ACCOUNTS (ACCOUNT_ID,BALANCE,TRANSACTION_ID) values('$staff_id','0.00','0')";
				DBQuery($sql);
}
				// associate with default food service account and assign other defaults
				$sql = "INSERT INTO FOOD_SERVICE_STAFF_ACCOUNTS (STAFF_ID,STATUS,BARCODE,BALANCE) values('$staff_id','','','0.00')";
				DBQuery($sql);
			}
*/
#########################################################################################################
