...
#################### code differ for windows and Linux machine ########################
			...
			{
				$sql .= $column."='".str_replace("'", "\'",$value)."',"; // for linux Machine----kajal
				#$sql .= $column."='".$value."',";    // for Windows Machine --------- kajal
				#$values .= "'".$value."',";  
			}
		...
		else
		{
			...
			foreach($columns as $column=>$value)
			{
				...
				$values .= "'".str_replace("'", "\'",$value)."',";   // for linux machine -------kajal
				 #$values .= "'".$value."',";      // for windows machine---------kajal
