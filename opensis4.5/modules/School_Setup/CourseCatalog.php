...
if($_REQUEST['create_pdf']=='true')
{
    ...
    PDFStop($handle, 'ani.pdf');
    exit();
}

if($_REQUEST['create_excel']=='true')
{
...
else
{
    ...
    if($_REQUEST['print']!='list')
    {
        echo "<FORM name=search id=search action=for_export.php?modname=$_REQUEST[modname]&modfunc=print&marking_period_id=".$_REQUEST['marking_period_id']."&_CENTRE_PDF=true&report=true&print=list method=POST target=_blank>";
...
function CreateList($dli='', $pli='', $sli='', $cli='', $mp='', $mp_name='')
{
    ...
    $html = "<b>".$heading."</b><br>
        <A HREF=".str_replace('Modules.php', 'for_export.php', $PHP_tmp_SELF)."&create_excel=true&LO_save=1&_CENTRE_PDF=true > <IMG SRC=assets/download.png border=0 vspace=0 hspace=0></A>
        <br>";
    ...
    $columns = array('SUBJECT'=>'Subject','COURSE'=>'Course','PERIOD'=>'Time','DAYS'=>'Days','LOCATION'=>'Location','TEACHER'=>'Teacher');
